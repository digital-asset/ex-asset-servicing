...
const CodeAllocations : React.FC<RouteComponentProps> = ({ history } : RouteComponentProps) => {
  ...
  const entries = useStreamQueries(CodeAllocationRequest).contracts;

  const respondCodeAllocation = async (codeAllocationRequestCid : ContractId<CodeAllocationRequest>) => {
    const allocatedCode = "XS" + Math.round(Math.random() * 9999999999);
    await ledger.exercise(CodeAllocationRequest.RespondCodeAllocation, codeAllocationRequestCid, { allocatedCode });
  };
        ...
        {entries.map((e, i) => (
          <TableRow key={i} className={classes.tableRow}>
            ...
            <TableCell key={9} className={classes.tableCell}>
              <Button color="secondary" size="small" className={classes.choiceButton} variant="contained" onClick={() => respondCodeAllocation(e.contractId)}>Allocate</Button>
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
...
export default withRouter(CodeAllocations);